...
const KestraSetup = () => {
  ...
  const generateDockerCommand = (os: "macos" | "linux") => {
    ...
    // Single-line for clipboard
    const singleLine = `docker run --pull=missing --rm -it -p 8080:8080 --user=root ${javaOpts}${javaOpts ? ' ' : ''}-v /var/run/docker.sock:/var/run/docker.sock -v /tmp:/tmp -e "SECRET_OPENAI_API_KEY=$(echo -n '${apiKey || 'your-api-key-here'}' | base64)" -e "SECRET_WEBHOOK_SECRET_KEY=$(echo -n '${webhookSecret || 'your-webhook-secret'}' | base64)" kestra/kestra:latest server local`;
    
    // Multi-line for display
    const displayFormat = `docker run --pull=missing --rm -it -p 8080:8080 \\
  --user=root${javaOpts ? ` \\
  ${javaOpts}` : ''} \\
  -v /var/run/docker.sock:/var/run/docker.sock \\
  -v /tmp:/tmp \\
  -e "SECRET_OPENAI_API_KEY=$(echo -n '${apiKey || 'your-api-key-here'}' | base64)" \\
  -e "SECRET_WEBHOOK_SECRET_KEY=$(echo -n '${webhookSecret || 'your-webhook-secret'}' | base64)" \\
  kestra/kestra:latest server local`;
    
    return { singleLine, displayFormat };
  };

  const getWorkflowYaml = () => {
    const yaml = `id: issue-assignment-automation
namespace: autoaudit
      ...
      REJECT:
        - id: generate_response
          ...
          script: |
            ...
            comment_json = json.dumps(dynamic_comment)
            Kestra.outputs({'comment': dynamic_comment, 'comment_json': comment_json})
        ...
        - id: request_more_details
          ...
          body: |
            {
              "comment": {{ outputs.generate_response.vars.comment_json }},
              "githubToken": "{{ trigger.body.github_token }}",
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <Card className="mb-6">
            <CardHeader>
              ...
              <CardDescription>
                Provide your API key and set up authentication
              </CardDescription>
            </CardHeader>
            ...
            <CardContent>
              <Tabs defaultValue="linux" className="w-full">
                ...
                <TabsContent value="linux" className="space-y-4">
                  <div className="relative">
                    <pre className="bg-black/95 text-green-400 p-4 rounded-lg overflow-x-auto text-sm font-mono">
                      <code>{generateDockerCommand("linux").displayFormat}</code>
                    </pre>
                    <Button
                      size="sm"
                      variant="ghost"
                      className="absolute top-2 right-2"
                      onClick={() => copyToClipboard(generateDockerCommand("linux").singleLine, "linux")}
                    >
                      {copiedCommand === "linux" ? (
                ...
                <TabsContent value="macos" className="space-y-4">
                  <div className="relative">
                    <pre className="bg-black/95 text-green-400 p-4 rounded-lg overflow-x-auto text-sm font-mono">
                      <code>{generateDockerCommand("macos").displayFormat}</code>
                    </pre>
                    <Button
                      size="sm"
                      variant="ghost"
                      className="absolute top-2 right-2"
                      onClick={() => copyToClipboard(generateDockerCommand("macos").singleLine, "macos")}
                    >
                      {copiedCommand === "macos" ? (
                        <Check className="w-4 h-4 text-green-500" />
                      ) : (
